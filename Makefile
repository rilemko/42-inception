...
#                                                 +#+#+#+#+#+   +#+            #
#    Created: 2024/04/03 12:46:30 by mconreau          #+#    #+#              #
#    Updated: 2024/04/03 12:46:32 by mconreau         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
build					:
							@if [ ! -f srcs/.env ]; then\
								if [ $(REPORT_ERRORS) = "yes" ]; then\
									...
									printf "$(MSG_BCOLOR_FAILURE) Unable to find \".env\" file !\n";\
									printf "$(MSG_BCOLOR_FAILURE)[ $(MSG_SYMBOL_FAILURE) ]";\
									printf "$(MSG_NCOLOR_LOGGING)$(V_DISPLAY_NAME)";\
									if [ $(REPORT_TIMESTAMPS) = "yes" ]; then\
										printf "$(MSG_NCOLOR_LOGGING) $(O_CURRENT_TIME) |";\
									fi;\
									printf "$(MSG_BCOLOR_FAILURE) Rename \".env.example\" to \".env\" and replace necessary information.\n";\
							...
							@docker compose -p $(PROJECT_NAME) -f $(PROJECT_SOURCES) up --build -d
...
.PHONY					:	all start stop restart ps cclean fclean vclean down re
